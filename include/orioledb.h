...
/* What versions in OrioleDB code mean.
 *
 * Currently OrioleDB has the following version-related values: ORIOLEDB_VERSION, ORIOLEDB_WAL_VERSION, ORIOLEDB_CHECKPOINT_CONTROL_VERSION, ORIOLEDB_BINARY_VERSION, ORIOLEDB_SYS_TREE_VERSION, ORIOLEDB_PAGE_VERSION, and ORIOLEDB_COMPRESS_VERSION.
 *
 * - ORIOLEDB_VERSION - is an external reference-only value meaning the current release. It is output by orioledb_version() SQL function and
 * its change doesn't mean any changes in the code that can introduce incompatibilities.
 *
 * The following versions are made to reflect incompatibilities and limitations:
 *
 * - ORIOLEDB_WAL_VERSION - Version of OrioleDB WAL format (see include/recovery/wal.h)
 *
 * ORIOLEDB_WAL_VERSION is used for on-the fly conversion of WAL. As WAL can be transferred between different clusters ORIOLEDB_WAL_VERSION compatibility is not limited to the same ORIOLEDB_BINARY_VERSION (see below). Compatibility is only one-way, if read versions are lower than current, WAL will be converted seamlessly at its reading. But if read versions are greater than current there is a difference for WAL used in recovery and WAL used for logical decoding.
 ...
 *
 * - ORIOLEDB_CHECKPOINT_CONTROL_VERSION - Version of OrioleDB control file format (see include/checkpoint/control.h)
 *
 * ORIOLEDB_CHECKPOINT_CONTROL_VERSION is used for on-the fly conversion of CheckpointControl structure. Compatibility is only one-way, if read versions are lower than current CheckpointControl will be converted seamlessly. Otherwise cluster will refuse to start with error. Note that CheckpointControl structure is used for reading cluster's ORIOLEDB_BINARY_VERSION(see below)
 *
 * - ORIOLEDB_BINARY_VERSION - Clusters with different ORIOLEDB_BINARY_VERSION are binarily incompatible.
 *
 * ORIOLEDB_BINARY_VERSION of a cluster is written to a checkpoint control file. At start OrioleDB will check if current ORIOLEDB_BINARY_VERSION is equal to what is in the control file. Otherwise the cluster will refuse to start. (We don't go to the following versions checks in this case)
 *
 ...
 *
 * These ORIOLEDB_SYS_TREE_VERSION, ORIOLEDB_PAGE_VERSION, ORIOLEDB_COMPRESS_VERSION reflect incompatibilities that could be converted on-the-fly. Compatibility is only one-way: if read versions are greater than current, cluster will shut down with error. If read version is lower than current - seamless conversion will occur at the first reading.
 *
 * As said above, these values are not checked if ORIOLEDB_BINARY_VERSION is different, so each of these values makes sense only within one ORIOLEDB_BINARY_VERSION value.
 */
