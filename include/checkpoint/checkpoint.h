...
typedef struct
{
	 ...
	 * WAL_REC_O_TABLES_META_LOCK..WAL_REC_O_TABLES_META_UNLOCK by buffering the
	 * enclosed O_TABLES/O_INDICES modifies and applying them atomically at the
	 * UNLOCK; while a window is open the sys trees show the pre-DDL state.  The
	 * checkpointer clamps replayStartPtr/sysTreesStartPtr to this value so
	 * recovery never restarts inside an open (buffered) window and thus never
	 * loses the buffered modifies.
	 */
