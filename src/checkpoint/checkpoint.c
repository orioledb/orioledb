...
o_perform_checkpoint(XLogRecPtr redo_pos, int flags)
{
	 ...
	 * trees we are about to checkpoint show the pre-DDL state), start
	 * recovery no later than that window's WAL_REC_O_TABLES_META_LOCK.
	 * Otherwise a restart would begin inside the window, miss the META_LOCK,
	 * and lose the buffered modifies.  0 means no open window (always so on
	 * the primary).
	 */
	{
	 ...
	 * open (buffered) meta window, or its buffered O_TABLES/O_INDICES
	 * modifies would be skipped on restart.
	 */
	{
		XLogRecPtr	openWin = (XLogRecPtr)
			pg_atomic_read_u64(&checkpoint_state->oldestOpenMetaWindow);
...
checkpoint_ix(int flags, BTreeDescr *descr)
{
	...
	elog(DEBUG1, "checkpoint_map_write_header: (%u, %u) chkp=%u "
		 ...
		 "leafPagesNum=%u",
		 descr->oids.datoid, descr->oids.relnode, chkpNum,
						 ...
						 uint32 *map_chkp_num, bool clear_tree)
{
	...
	*evicted_data = read_evicted_data(desc->oids.datoid,
	...
	else
	{
		...
		{
			s3_load_map_file(prev_chkp_tag.num, prev_chkp_tag.key);
