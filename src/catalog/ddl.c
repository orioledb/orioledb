...
static void redefine_indices(Relation rel, OTable *new_o_table, bool primary, bool setting_tbl_tablespace, Oid oldRelnode);
...
redefine_indices(Relation rel, OTable *new_o_table, bool primary,
				 bool setting_tbl_tablespace, Oid oldRelnode)
{
	ListCell   *index;
		...
		if ((primary && ind->rd_index->indisprimary) || (!primary && !ind->rd_index->indisprimary))
		{
			...
			else
			{
				...
				o_define_index(rel, NULL, ind->rd_rel->oid, false,
							   InvalidIndexNumber, setting_tbl_tablespace, NULL);
				closed = true;
			}
		}
		if (!closed)
...
redefine_pkey_for_rel(Relation rel)
{
	...
	redefine_indices(rel, o_table, true, false, InvalidOid);
		...
		if (rel != NULL)
		{
					 ...
					 (subId == 0))
			{
				...
				if (is_orioledb_rel(tbl))
				{
					 ...
					 * rewrite_table
					 */
					redefine_indices(tbl, new_o_table, true, false, InvalidOid);
					...
					redefine_indices(tbl, new_o_table, false, false, InvalidOid);
					 ...
					 (subId == 0))
			{
				...
				if (tbl && is_orioledb_rel(tbl))
				{
					...
					if (o_saved_reltablespace != rel->rd_rel->reltablespace)
					{
						 ...
						 * rewrite_table
						 */
						redefine_indices(tbl, new_o_table, true, true, old_o_table->oids.relnode);
						...
						redefine_indices(tbl, new_o_table, false, true, old_o_table->oids.relnode);
...
#undef PG_TBLSPC
	 ...
	 * objects in pg_constraint rather than just a flag on pg_attribute.
	 * We intercept constraint changes to keep the OrioleDB cache in sync.
	 *
			 ...
			 classId == ConstraintRelationId)
	{
		 ...
		 * the metadata unnecessarily:
		 * - ALTER TABLE <table> DROP COLUMN <col>;
		 * - DROP TYPE <type> CASCADE;
		 */

		/* 
		 * Ensure catalog tuples inserted/updated by the current command 
		 * are visible to SearchSysCache1.
		 */
		...
		if (HeapTupleIsValid(conTup))
		{
			...
			if (conForm->contype == CONSTRAINT_NOTNULL && conForm->conrelid != InvalidOid)
			{
				...
				int16	   *attnums;
				int			numkeys;
				Relation	rel = relation_open(conForm->conrelid, AccessShareLock);
				...
				if (is_orioledb_rel(rel))
				{
					...
					if (o_table != NULL)
					{
						...
						/*
						 * Directly update the notnull flag on the OrioleDB table metadata.
						 * We avoid delegating to the relation attribute hook because
						 * rel->rd_att may not be fully updated yet during constraint creation.
						 */
						...
						if (changed)
						{
							/* Persist the updated configuration to the OrioleDB catalog tree */
							o_indices_update(o_table, PrimaryIndexNumber, oxid, oSnapshot.csn);
							o_tables_update(o_table, oxid, oSnapshot.csn);
							o_tables_after_update(o_table, oxid, oSnapshot.csn);
						}
