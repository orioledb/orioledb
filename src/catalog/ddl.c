							...
							int subId, void *arg)
{
	...
	{
		/*
		 * Move all the orioledb objects from the source tablespace to the destination
		 * tablespace.
		 * At this point CHECKPOINT was done and there is no new connection to the moved
		 * database because LockSharedObjectForSession on database was done as well.
		 */
...
static void o_check_movedb(const AlterDatabaseStmt *stmt, movedb_params *movedb)
{
	...
	if (!get_db_info(stmt->dbname, NoLock, &db_id))
		/*
		 * Lets standard processing of moving datatabase
		 * generates an error.
		 */
		return;
	...
	foreach(option, stmt->options)
	{
		...
		{
			/* skip check options because it will done in core statement handling
			* before
