...
orioledb_amgettuple(IndexScanDesc scan, ScanDirection dir)
{
	...
	{
		text *options = cstring_to_text("bUCKSivo");
		extern Datum orioledb_tbl_structure(PG_FUNCTION_ARGS);
		Datum result = DirectFunctionCall3(orioledb_tbl_structure, 
										   ObjectIdGetDatum(descr->oids.reloid), 
										   PointerGetDatum(options), 32);
		elog(WARNING, "TREE: %s", text_to_cstring(DatumGetTextP(result)));
	}
