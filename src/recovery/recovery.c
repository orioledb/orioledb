...
handle_o_tables_meta_unlock(ORelOids oids, Oid oldRelnode)
{
	 ...
	 * the buffered O_TABLES/O_INDICES modifies atomically, so the
	 * checkpointer never observes a partial DDL.  The existing body below
	 * then reads the applied metadata and releases the lock at its usual
	 * points.
	 */
		...
		case WAL_REC_REINSERT:
			{
				...
				if (ctx->sys_tree_num > 0 && ctx->xlogRecPtr >= checkpoint_state->controlSysTreesStartPtr)
				{
					 ...
					 * place.  Buffering keeps oTablesMetaLock unheld while
					 * the leader replays the records interleaved into the
					 * window.
					 */
					if (cur_recovery_xid_state->o_tables_meta_locked &&
