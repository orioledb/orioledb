...
#include "postgres.h"			/* Still needed for headers below to use Datum */
...
static int
bsearch_insertion_compare(const void *a, const void *b)
{
...
start_target_postgres(const char *argv0, const char *datadir, bool debug)
{
	...
	pg_ctl_args =
		(const char *[])
	{
		"start", "-D", datadir, "-l", log_file, NULL
	};
...
{
	const char *args[] = {"stop", "-D", datadir, NULL};

	run_pg_ctl(argv0, args);
}
...
static WalParseResult
pg_rewind_on_record(WalReaderState *r, WalRecord *rec)
{
	OrioledbKeyMap *orioledb_map = (OrioledbKeyMap *) r->ctx;
	...
	switch (rec->type)
	{
		...
		case WAL_REC_RELATION:
			{
				...
				{
					orioledb_map->tree_key.type = rec->u.relation.treeType;
		...
		case WAL_REC_REINSERT:
			{
				/*
				 * Check if works without this for now; TODO:	add support for
				 * read_two_tuples
				 */
				Assert(!rec->u.modify.read_two_tuples);
				if (orioledb_map->fill_map)
				{
					OrioledbKey *key;

					key = create_orioledb_key(rec->u.modify.len1,
											  rec->u.modify.t1.data,
											  rec->u.modify.t1.formatFlags);
...
extract_row_info(XLogReaderState *record, void *arg)
{
	...
	if (rmid == ORIOLEDB_RMGR_ID)
	{
		...
		WalReaderState r = {
			...
			.container = {0},
			.ctx = arg,
			.check_version = pg_rewind_check_version,
			.on_container = NULL,
			.on_record = pg_rewind_on_record
		};
		WalParseResult st;
		...
		if (st != WALPARSE_OK)
			pg_log_error("[ORIOLEDB WAL PARSE ERROR %d]", st);
	}
}
