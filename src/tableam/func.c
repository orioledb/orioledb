						 ...
						 bool print_bytes, int depthLeft, StringInfo outbuf)
{
	...
	for (loc.chunkOffset = 0; loc.chunkOffset < header->chunksCount; loc.chunkOffset++)
	{
		...
		appendStringInfo(outbuf, "[%d]: %d\n",
						 loc.chunkOffset, pageContext.tupleChunk->chunkDataSize);
		...
		for (OffsetNumber itemOffset = 0;
			 itemOffset < pageContext.tupleChunk->chunkItemsCount; itemOffset++)
		{
			appendStringInfoSpaces(outbuf, level * 4);
		...
		align = MAXALIGN(sizeof(BTreeChunkItem) * pageContext.tupleChunk->chunkItemsCount) -
			sizeof(BTreeChunkItem) * pageContext.tupleChunk->chunkItemsCount;
		if (align > 0)
		{
			appendStringInfoSpaces(outbuf, level * 4);
		...
		for (loc.itemOffset = 0;
			 loc.itemOffset < pageContext.tupleChunk->chunkItemsCount;
			 loc.itemOffset++)
		{
			if (O_PAGE_IS(p, LEAF))
			{
...
static void
add_page_stat(BTreePageContext *pageContext, ORelationStat *stat)
{
