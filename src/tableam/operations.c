...
o_truncate_table(ORelOids oids)
{
	...
	for (int i = 0; i < treeOidsNum; i++)
	{
		...
		desc = index_oids_get_btree_descr(treeOids[i].oids, treeOids[i].type);
		Assert(desc != NULL);

		cleanup_btree(desc, treeOids[i].oids.datoid, treeOids[i].oids.relnode, true);
...
/*		if (is_recovery_process())
	...
	if (!invalidatedTable)
	{
		...
		desc = index_oids_get_btree_descr(oids, oIndexPrimary);
		Assert(desc != NULL);

		cleanup_btree(desc, oids.datoid, oids.relnode, true);
