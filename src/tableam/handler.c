						   ...
						   TupleTableSlot *slot)
{
	...
	{
		/*
		 * TEMP ORI217 instrumentation: this NULL makes the executor raise
		 * "failed to fetch tuple being updated" (nodeModifyTable.c).  The
		 * re-fetch reads the row at the CSN embedded in the rowid
		 * (rowid_set_csn(marg.csn) on a TM_Updated).  Log the rowid CSN +
		 * version, the visibility outcome, the CSN horizons, and re-probe the
		 * same PK at the latest (in-progress) snapshot to tell "invisible at
