								...
								TableScanDesc scan)
{
	...
	if (indexRelation->rd_rel->relam != BTREE_AM_OID || (options && !options->orioledb_index))
	{
		...
		/*
		 * TableScanDesc scan is unused in this function but could be
		 * passed by a caller (e.g in a case of parallel bridged index
		 * build) We need to close it here, same as in
		 * heapam_index_build_range_scan. Otherwise BTreeSeqScan leaks
		 * until ResourceOwner release warns "resource was not closed".
		 */
		if (scan)
			table_endscan(scan);
