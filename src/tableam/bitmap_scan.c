		...
		if (!O_TUPLE_IS_NULL(tuple))
		{
			...
			else
			{
				if (indexDescr->desc.type != oIndexPrimary)
				{
					...
					Datum		val;
					bool		is_null;
					BTreeKeyType keyType = BTreeKeyLeafTuple;
					...
					{
						attnum = OIndexKeyAttnumToTupleAttnum(keyType, primary, ctid_off);
...
add_rbt_to_tbm(OBitmapHeapPlanState *bitmap_state, TIDBitmap *tbm, RBTree *rbt)
{
	...
	while (true)
	{
		...
		else
		{
			...
			attnum = OIndexKeyAttnumToTupleAttnum(keyType, primary, 0);
			val = o_toast_nocachegetattr(tuple, attnum, tupdesc, spec, &is_null);
			Assert(!is_null);
			bridge_iptr = DatumGetItemPointer(val);
	...
	do
	{
		...
		if (scan->tbmiterator != NULL && scan->tbmres == NULL)
		{
			...
			if (!fetched)
			{
				...
				{
					/*
					 * Bitmap is non-lossy, so we just look through the
					 * offsets listed in tbmres; but we have to follow any HOT
					 * chain starting at each such offset.
					 */
					int			curoff;
				...
				{
					/*
					 * Bitmap is lossy, so we must examine each line pointer
					 * on the page.
					 */
