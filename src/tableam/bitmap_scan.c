...
bridge_iterate(BridgeIterator *iter)
{
	...
	if (!BlockNumberIsValid(iter->tbmres.blockno))
	{
		...
		if (!iter->tbmres.lossy)
			iter->page_ntuples = tbm_extract_page_tuple(&iter->tbmres,
														iter->offsets,
	...
	if (iter->tbmres == NULL)
	{
		...
		if (iter->tbmres)
			iter->page_ntuples = iter->tbmres->ntuples;
	}
...
bridge_next_page(OBitmapScan *scan, OBitmapHeapPlanState *bitmap_state)
{
	...
	if (iter->page_ntuples >= 0)
	{
		...
		for (curoff = 0; curoff < iter->page_ntuples; curoff++)
		{
#if PG_VERSION_NUM >= 180000
