...
struct BTreeSeqScan
{
	...
	BTreePageContext leafPageContext;
	BTreePageContext histPageContext;
...
static void get_next_key(BTreePageContext *pageContext,
...
load_first_historical_page(BTreeSeqScan *scan)
{
	...
	if (!O_PAGE_IS(scan->leafImg, RIGHTMOST))
		copy_fixed_hikey(&scan->leafPageContext, &hikey);
	else
		   ...
		   header->csn >= scan->oSnapshot.csn)
	{
		...
		page_context_set_invalid(&scan->histPageContext);

		if (!O_PAGE_IS(scan->histImg, RIGHTMOST))
			copy_fixed_hikey(&scan->histPageContext, &hikey);
		else
			O_TUPLE_SET_NULL(hikey.tuple);
...
load_next_historical_page(BTreeSeqScan *scan)
{
	...
	copy_fixed_hikey(&scan->histPageContext, &prevHikey);
						...
						OffsetNumber *startOffset)
{
	...
	page_context_set_invalid(destPageContext);
	...
	else
	{
		...
		page_context_set_invalid(&scan->leafPageContext);
...
static void
get_current_downlink_key(BTreePageContext *pageContext,
						 ...
						 uint64 *downlink)
{
	...
	STOPEVENT(STOPEVENT_STEP_DOWN, btree_downlink_stopevent_params(pageContext->treeDesc,
																   pageContext->page, loc));
...
static void
get_next_key(BTreePageContext *pageContext,
			 BTreePageItemLocator *intLoc, OFixedKey *nextKey)
{
	...
	if (BTREE_PAGE_LOCATOR_IS_VALID(pageContext->page, intLoc))
		...
		copy_fixed_hikey(pageContext, nextKey);
	else
				  ...
				  OFixedKey *keyRangeLow, OFixedKey *keyRangeHigh)
{
		...
		while (true)
		{
			...
			if (!pageIsLoaded)
			{
				if (scan->firstPageIsLoaded)
				{
					...
					copy_fixed_hikey(&scan->context.pageContext,
									 &scan->prevHikey);
				}
	...
	{
		BTreePageContext curPageContext,
						nextPageContext;

		page_context_init(&curPageContext, scan->desc);
		page_context_init(&nextPageContext, scan->desc);
		...
		while (true)
		{
			...
			page_context_set_page(&curPageContext, curPage->img);
			page_context_set_page(&nextPageContext, nextPage->img);

			if (poscan->flags & O_PARALLEL_IS_SINGLE_LEAF_PAGE)
			{
				page_context_release(&curPageContext);
				page_context_release(&nextPageContext);
				...
				!O_PAGE_IS(curPage->img, RIGHTMOST))
			{
				...
				copy_fixed_shmem_hikey(&curPageContext, &nextPage->prevHikey);
...
check_in_memory_leaf_page(BTreeSeqScan *scan, OTuple keyRangeLow, OTuple keyRangeHigh)
{
	...
	if (!O_PAGE_IS(scan->leafImg, RIGHTMOST))
		leafHikey = page_get_hikey(&scan->leafPageContext);
	else
...
init_btree_seq_scan(BTreeSeqScan *scan)
{
	...
	page_context_set_page(&scan->leafPageContext, scan->leafImg);
	page_context_set_page(&scan->histPageContext, scan->histImg);
							 ...
							 BlockSampler sampler, ParallelOScanDesc poscan)
{
	...
	page_context_init(&scan->leafPageContext, desc);
	page_context_init(&scan->histPageContext, desc);
		...
		while (scan->haveHistImg)
		{
			...
			while (!BTREE_PAGE_LOCATOR_IS_VALID(scan->histImg, &scan->histLoc))
			{
				...
				if (!O_PAGE_IS(scan->leafImg, RIGHTMOST))
				{
					...
					leafHikey = page_get_hikey(&scan->leafPageContext);
					histHikey = page_get_hikey(&scan->histPageContext);
					if (o_btree_cmp(scan->desc,
...
free_btree_seq_scan(BTreeSeqScan *scan)
{
	...
	page_context_release(&scan->leafPageContext);
	page_context_release(&scan->histPageContext);
