...
init_checkpoit_number(BTreeSeqScan *scan)
{
	...
	/*
	 * Make sure the meta page this registers against is the tree's current
	 * one.  A descriptor is cached per backend while nothing keeps the tree
	 * loaded: the clock sweep can evict it and hand the meta page to somebody
	 * else, and the descriptor goes on naming it.  Counting a scan into a
	 * page that belongs to another tree now is issue #1133; counting it into
	 * one that is on its way out loses the answer
	 * can_use_checkpoint_extents() needs.
	 *
	 * find_page() asks the same question on the way down and drops the
	 * descriptor before reloading, which is what this does -- here rather
	 * than inside o_btree_load_shmem(), because reloading allocates pages and
	 * its other callers must be able to go on assuming that reaching a loaded
	 * tree does not.
	 */
	if (desc->storageType != BTreeStorageTemporary &&
		ORootPageIsValid(desc) && OMetaPageIsValid(desc) &&
		!(BTREE_ROOT_PAGE_IS_OURS(desc) && BTREE_META_PAGE_IS_OURS(desc)))
	{
