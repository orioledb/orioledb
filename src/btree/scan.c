				  ...
				  OFixedKey *keyRangeLow, OFixedKey *keyRangeHigh)
{
	...
	else
	{
		...
		btree_page_context_init(&curPageContext, scan->desc);
		btree_page_context_init(&nextPageContext, scan->desc);
							 ...
							 BlockSampler sampler, ParallelOScanDesc poscan)
{
	...
	btree_page_context_init(&scan->leafPageContext, desc);
	btree_page_context_init(&scan->histPageContext, desc);
