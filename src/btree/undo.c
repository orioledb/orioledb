							...
							bool changeCountsValid)
{
	...
	if (stage == OUndoCallbackStagePreCommit)
	{
		...
		/*
		 * Acquire here locks the commit stage below needs to drop the old trees.
		 *
		 * The commit stage runs from an XACT_EVENT_COMMIT callback, which PG's core
		 * calls between HOLD_INTERRUPTS() and RESUME_INTERRUPTS(), so the locks there
		 * cannot be interrupted.
		 *
		 * XACT_EVENT_PRE_COMMIT is called before the interrupt holdoff, so waiting here
		 * is safe and can be cancelled (using request cancel or timeout).  Once we hold
		 * the locks, the commit stage acquires them without waiting.
		 */
