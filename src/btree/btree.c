...
free_meta_page(OPagePool *pool, OInMemoryBlkno metaPageBlkno)
{
	...
	/*
	 * There might be non-released seq scans from the same transaction (other
	 * transactions are excluded by locks).  Defer freeing the meta page until
	 * the last scan is released.
	 */
