					  ...
					  bool checkpoint)
{
	...
	{
		page_context_release(&parentPageContext);
		page_context_release(&leftPageContext);
		page_context_release(&parentPageContext);
	...
	if (*merge_parent && is_page_too_sparse(desc, parent))
	{
		...
		if (!O_PAGE_IS(parent, RIGHTMOST))
			copy_fixed_hikey(&parentPageContext, parent_hikey);
		else
						   ...
						   bool nested, bool wait_io)
{
	...
	page_context_init(&targetPageContext, desc);
	page_context_set_page(&targetPageContext, target);

	/* copy hikey of current page */
	if (!O_PAGE_IS(target, RIGHTMOST))
		copy_fixed_hikey(&targetPageContext, &key);
	else
	...
	page_context_release(&targetPageContext);
			...
			CommitSeqNo csn)
{
	...
	leftHikeySize = page_get_hikey_size(leftPageContext);
	leftHikey = page_get_hikey(leftPageContext);
	...
	{
		rightHikeySize = page_get_hikey_size(rightPageContext);
		rightHikey = page_get_hikey(rightPageContext);
	}
