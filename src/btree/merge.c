...
static void merge_pages(BTreeDescr *desc, OInMemoryBlkno left_blkno,
						BTreePageLocator *leftPageContext, BTreePageLocator *rightPageContext,
						CommitSeqNo csn);
					  ...
					  bool checkpoint)
{
	...
	BTreePageLocator parentPageContext,
				leftPageContext,
				rightPageContext;
	...
	btree_page_locator_init(&parentPageContext, desc);
	btree_page_context_set(&parentPageContext, parent);
	btree_page_locator_hikey_init(&parentPageContext, &BTreeHiKeyChunkOps);

	btree_page_locator_init(&leftPageContext, desc);
	btree_page_context_set(&leftPageContext, left);
	btree_page_locator_hikey_init(&leftPageContext, &BTreeHiKeyChunkOps);

	btree_page_locator_init(&rightPageContext, desc);
	btree_page_context_set(&rightPageContext, right);
	btree_page_locator_hikey_init(&rightPageContext, &BTreeHiKeyChunkOps);
						   ...
						   bool nested, bool wait_io)
{
	...
	Page		target = O_GET_IN_MEMORY_PAGE(blkno),
				...
				right,
				left;
	BTreePageLocator targetPageContext;
	OFixedKey	key;
	int			level;
	...
	btree_page_locator_init(&targetPageContext, desc);
