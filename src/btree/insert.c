								 ...
								 bool lock)
{
	...
	keylen = BTREE_PAGE_GET_HIKEY_SIZE(leftPageContext->page);
	hikey = page_get_hikey(leftPageContext);
...
o_btree_fix_page_split(BTreeDescr *desc, OInMemoryBlkno left_blkno)
{
	...
	page_context_set_page(&leftPageContext, p);

	iitem.context = &context;
	copy_fixed_hikey(&leftPageContext, &key);
									 ...
									 OInMemoryBlkno left_blkno)
{
	...
	page_context_init(&pageContext, insert_item->context->desc);
