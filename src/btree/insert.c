...
static void o_btree_split_fill_downlink_item(BTreeInsertStackItem *insert_item,
											 BTreePageLocator *leftPageContext,
											 bool lock);
									 ...
									 OInMemoryBlkno left_blkno)
{
	...
	btree_page_locator_init(&pageContext, insert_item->context->desc);
...
o_btree_insert_item(BTreeInsertStackItem *insert_item, int reserve_kind)
{
	...
	while (insert_item != NULL)
	{
		...
		btree_page_context_set(&pageContext, p);
		btree_page_locator_hikey_init(&pageContext, &BTreeHiKeyChunkOps);
