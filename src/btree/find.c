		  ...
		  uint16 targetLevel)
{
	...
	while (true)
	{
		...
		/*
		 * Local-pool slots are NULLed on eviction, unlike shared-pool
		 * slots whose shmem page stays readable (only pageChangeCount
		 * changes).  An IN_MEMORY downlink we just descended through may
		 * reference a slot the backend evicted earlier in the same call
		 * chain -- e.g. a reserve_page triggered during a seq scan, or a
		 * find_page invoked from an undo callback.  PAGE_GET_LEVEL below
		 * would segfault on the NULL slot, so step back to the parent and
		 * re-resolve the downlink (it now points to disk).  At the root
		 * there is no parent, so report failure.
		 */
...
retry:
	...
	if (BTREE_PAGE_FIND_IS(context, MODIFY))
	{
		...
		/*
		 * Local-pool slots are NULLed on eviction, unlike shared-pool
		 * slots where pageChangeCount alone signals replacement (the
		 * shmem page stays readable).  The slot at the caller's saved
		 * (blkno, pageChangeCount) may have been evicted since, so
		 * PAGE_GET_LEVEL below would segfault.  Fall back to find_page()
		 * to resolve the downlink from scratch.
		 */
		if (O_PAGE_IS_LOCAL(intCxt.blkno) &&
			local_ppool_pages[intCxt.blkno & O_BLKNO_MASK] == NULL)
			return find_page(context, key, keyType, level);
