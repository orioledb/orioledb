					   ...
					   CommitSeqNo csn, uint16 flags)
{
	...
	page_context_init(&context->parentPageContext, desc);
	page_context_set_page(&context->parentPageContext, context->parentImg);
...
release_page_find_context(OBTreeFindPageContext *context)
{
	...
	page_context_release(&context->parentPageContext);
}
			...
			if (intCxt.haveLock)
			{
				...
				if (imageFlag && level == targetLevel + 1)
				{
					...
					memcpy(context->parentPageContext.page, intCxt.pageContext->page,
						   ORIOLEDB_BLCKSZ);
					page_context_set_invalid(&context->parentPageContext);
					context->partial.isPartial = false;
				}
			}
			else
...
follow_rightlink(OBTreeFindPageInternalContext *intCxt)
{
	...
	if (!O_PAGE_IS(intCxt->pageContext->page, RIGHTMOST))
		pageHiKey = page_get_hikey(intCxt->pageContext);
						...
						&pageHiKey, BTreeKeyNonLeafKey) >= followVal))
	{
		...
		if (BTREE_PAGE_FIND_IS(context, KEEP_LOKEY))
		{
			copy_fixed_hikey(intCxt->pageContext, &context->lokey);
		...
		if (intCxt->haveLock)
		{
			...
			page_context_set_page(intCxt->pageContext, O_GET_IN_MEMORY_PAGE(intCxt->blkno));
			intCxt->pageChangeCount = O_PAGE_GET_CHANGE_COUNT(intCxt->pageContext->page);
...
find_right_page(OBTreeFindPageContext *context, OFixedKey *hikey)
{
	...
	/* copy hikey */
	copy_fixed_hikey(&context->pageContext, hikey);
			...
			!BTREE_PAGE_FIND_IS(context, LOKEY_UNDO))
		{
			...
			if (next_lokey_loaded && BTREE_PAGE_LOCATOR_IS_VALID(context->parentImg, &loc))
			{
				...
				if (DOWNLINK_IS_IN_MEMORY(tuphdr->downlink))
				{
					...
					{
						imgHikey = page_get_hikey(&context->pageContext);
								...
								BTreeKeyType keyType)
{
	...
	while (high > low)
	{
		...
		midTup.data = page + SHORT_GET_LOCATION(header->chunkDesc[mid].hikeyShortLocation);
