...
ltc_get_item_flags(BTreeChunkDesc *chunk, OffsetNumber itemOffset)
{
	...
	return ITEM_GET_FLAGS(chunk->chunkItems.tupleItems[itemOffset]);
}
				   ...
				   uint8 flags)
{
	...
	chunk->chunkItems.tupleItems[itemOffset] =
		ITEM_SET_FLAGS(chunk->chunkItems.tupleItems[itemOffset], flags);
}
...
ltc_get_item(BTreeChunkDesc *chunk, OffsetNumber itemOffset)
{
	...
	return chunk->chunkData +
		ITEM_GET_OFFSET(chunk->chunkItems.tupleItems[itemOffset]);
}
...
ltc_get_item_size(BTreeChunkDesc *chunk, OffsetNumber itemOffset)
{
	...
	itemLocation = ITEM_GET_OFFSET(chunk->chunkItems.tupleItems[itemOffset]);

	if (itemOffset + 1 < chunk->chunkItemsCount)
		return ITEM_GET_OFFSET(chunk->chunkItems.tupleItems[itemOffset + 1]) -
			itemLocation;
	else
				  ...
				  uint16 itemSize)
{
	...
	for (int i = chunk->chunkItemsCount - 1; i > itemOffset; i--)
		chunk->chunkItems.tupleItems[i] = chunk->chunkItems.tupleItems[i - 1] + dataShift;
	chunk->chunkItems.tupleItems[itemOffset] = itemPtr - chunk->chunkData + itemsShift;
}
...
ltc_init(BTreeChunkDesc *chunk, Page page, OffsetNumber chunkOffset)
{
	...
	chunk->chunkItems.tupleItems = (BTreeChunkItem *) chunk->chunkData;
...
ltc_compact(BTreeChunkDesc *chunk)
{
		 ...
		 itemOffset++)
	{
		...
		if (((BTreeLeafTuphdr *) header)->deleted == BTreeLeafTupleNonDeleted)
			chunk->chunkItems.tupleItems[lastOffset++] =
				chunk->chunkItems.tupleItems[itemOffset];
	}
