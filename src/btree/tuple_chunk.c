...
static void insert_chunk_item(BTreeChunkDesc *desc, OffsetNumber itemOffset,
							  uint16 itemSize);
						   ...
						   Pointer tupleHeader, OTuple tuple)
{
	...
	if (operation == BTreeChunkOperationInsert)
	{
		...
		insert_chunk_item(desc, itemOffset,
						 ...
						 BTreeChunkType chunkType, Size size)
{
	...
	builderData->chunkData = palloc(size);
	builderData->chunkDataMaxSize = size;
...
static void
insert_chunk_item(BTreeChunkDesc *desc, OffsetNumber itemOffset,
				  uint16 itemSize)
{
	LocationIndex itemsShift,
				dataShift;
