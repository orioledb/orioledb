				 ...
				 LocationIndex header_size, Page new_page)
{
	...
	btree_page_context_invalidate(&stack[level].pageContext);
					   ...
					   CheckpointFileHeader *file_header)
{
	...
	int			root_level = 0,
				saved_root_level;
	BTreePageLocator *rootPageContext;
	uint64		downlink;
	...
	for (i = 0; i < ORIOLEDB_MAX_DEPTH; i++)
	{
		...
		btree_page_locator_init(&stack[i].pageContext, desc);
		btree_page_context_set(&stack[i].pageContext, stack[i].img);
	}
	...
	btree_page_context_invalidate(rootPageContext);
	...
	for (i = 0; i < ORIOLEDB_MAX_DEPTH; i++)
		btree_page_context_release(&stack[i].pageContext);
