...
PG_FUNCTION_INFO_V1(test_leaf_tuple_chunk);
PG_FUNCTION_INFO_V1(test_leaf_tuple_chunk_builder);
PG_FUNCTION_INFO_V1(test_hikey_chunk);
PG_FUNCTION_INFO_V1(test_hikey_chunk_builder);
...
ltc_init_desc(TupleChunkTestDesc *testDesc)
{
	...
	testDesc->chunk = make_chunk_desc(&testDesc->indexDesc->desc,
									  &LeafTupleChunkOps,
									  testDesc->page, 0);
}
...
{
	testDesc->chunkBuilder = make_chunk_builder(&testDesc->indexDesc->desc,
												&LeafTupleChunkOps);
}
...
Datum
test_leaf_tuple_chunk(PG_FUNCTION_ARGS)
{
...
Datum
test_leaf_tuple_chunk_builder(PG_FUNCTION_ARGS)
{
	Oid			relid = PG_GETARG_OID(0);
	TupleChunkTestDesc testDesc;
...
Datum
test_hikey_chunk(PG_FUNCTION_ARGS)
{
	Oid			relid = PG_GETARG_OID(0);
	TupleChunkTestDesc testDesc;
	BTreeChunkDesc *chunk;
	...
	for (OffsetNumber itemOffset = 0;
		 itemOffset < chunk->chunkItemsCount; itemOffset++)
	{
		test_read_tuple(&testDesc, itemOffset, false);
	}
...
Datum
test_hikey_chunk_builder(PG_FUNCTION_ARGS)
{
	Oid			relid = PG_GETARG_OID(0);
	TupleChunkTestDesc testDesc;
