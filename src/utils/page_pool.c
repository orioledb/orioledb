				...
				volatile sig_atomic_t *shutdown_requested)
{
	...
	skippedLocalEvictionsLimit = (uint64) pool->size * UCM_USAGE_LEVELS;
	lastPageEvictSharedCount = pg_atomic_read_u64(pool->pageEvictCount);
	...
	while (true)
	{
		...
		{
			Assert(!have_locked_pages());
			pg_atomic_fetch_add_u64(pool->pageEvictCount, 1);
			break;
		}
		Assert(!have_locked_pages());
		...
		{
			uint64		currentPageEvictSharedCount = pg_atomic_read_u64(pool->pageEvictCount);
