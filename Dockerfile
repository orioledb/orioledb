...
# With older versions of Python3, the tests may fail
...
# In this case, each image size exceeds 1-2GB
# If RUN_REGRESSION_TESTS=yes, this option will run the regression tests with " .. || true ",
#  meaning it will not stop on errors - so perfect for debugging.
...
RUN set -eux; \
	...
	echo "RUN_REGRESSION_TESTS=$RUN_REGRESSION_TESTS" ; \
...
#		--with-pam \
	...
	if [[ "${RUN_REGRESSION_TESTS}" == "yes" ]]; then \
		# Install test dependencies and remove them after the tests
		# Keep in sync with ./ci/post_build_prerequisites.sh
		set -eux; \
		apk add --no-cache --virtual .check-deps py3-pip py3-virtualenv; \
		virtualenv /tmp/env ; \
		source /tmp/env/bin/activate ; \
		pip install --no-cache-dir --upgrade pip ; \
		pip install --no-cache-dir --upgrade psycopg2 six testgres moto[s3] flask flask_cors boto3 pyOpenSSL ; \
		pip freeze ; \
		...
		make install ; \
		cd .. ; \
		rm -r wal2json-wal2json_2_6 ; \
		\
		# Run the regression tests
		echo "nproc=$(nproc)" ; \
		# Known issue: Tests may fail intermittently with Python versions <= 3.10.
		su postgres -c 'source /tmp/env/bin/activate && make -C contrib/orioledb check -j$(nproc) LANG=C PGUSER=postgres' || [ "$DEBUG_MODE" = "true" ] ; \
		\
		# Clean up only if not in debug mode
		if [ "$DEBUG_MODE" != "true" ]; then \
			# Uninstall wal2json
			rm /usr/local/lib/postgresql/wal2json.so ; \
