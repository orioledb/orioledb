...
4. There are yet no safeguards against interruption of rewind when it's already started.  This interruption can leave the database in an inconsistent, partially rewound state.
5. Rewind is not yet compatible with replication, as rewind buffer state is not fixed into WAL, so any transactions rewound on the source will still look as committed on the replica.
