...
OrioleDB provides the ability to rewind database state to a some point in the past. This is achieved by using the undo log that records modifications for all ```orioledb``` tables. For heap tables this is used by delaying vacuum, so that while the transaction is not past the rewind boundary, the past versions of tuples are retained in the heap table, being invisible to the user.
