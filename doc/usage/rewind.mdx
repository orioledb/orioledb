...
To rewind run one of the queries:

- `select orioledb_rewind_by_time(1000);`  -- rewind all transactions committed after 1000 seconds before present
- `select orioledb_rewind_to_timestamp(<timestamptz>);` -- rewind all transactions committed after specified timestamp
- ```
...
Trying to rewind past time or xid theshold will output error. Trying to rewind to xid/oxid past max_transactions treshold will rewind upto maximum number of transactions.
...
All configuration options for resind
