...
index and OrioleDB table is automatically added at the first non-btree index build. 
...
Manual build of a bridge index for a table is possible but not necessary:
```
ALTER TABLE blog_post SET (index_bridging);
```

If all existing indexes for a table were removed, the "bridge index" is not removed automatically. If you don't plan to add non-btree indexes anymore you can delete unnecessary "bridge index" for this table:
```
