...
Maximum number of rewind transaction items that are stored for a rewind. Older items are completed and removed from the buffer sothis specifies maximum rewind age. If there are subtransactions they also occupy items in a rewind buffer.

`orioledb.rewind_max_transactions` and `orioledb.rewind_max_time` work together so if a transaction is older than any of these thresholds it is completed and could not be rewound anymmore.
...
Number of orioledb engine in-memory rewind buffers in blocks. Each block accommodate 68 rewind transaction items. If you have enogh memory set it near a value `orioledb.rewind_max_transactions` / 68 to have all rewind buffers in memory.
