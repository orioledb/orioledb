...
O_MAX_UNDO_RECORD_SIZE` bytes, so the actual buffer at startup may be larger than
...
- `table_lock` (default): take a heavyweight `ExclusiveLock` on every relation a SERIALIZABLE transaction touches. Two SERIALIZABLE transactions on the same table serialize via the lock manager, and SERIALIZABLE writers block concurrent non-SERIALIZABLE writers (`RowExclusiveLock`) on that table; non-SERIALIZABLE readers (`AccessShareLock`) are unaffected. Correct, but coarse — concurrent non-conflicting workloads degrade to serial execution.
...
Size of the per-backend local page pool used for OrioleDB temporary tables. Increase it to improve performance with
large temporary tables. This memory is allocated in each backend's private memory, not in shared memory, so memory
consumption will scale with the number of backends that touch OrioleDB temporary tables.
...
Return space freed by copy-on-write to the filesystem by punching holes in the data files.

OrioleDB writes a modified page to a new location and frees the old one, and a freed block only becomes
reusable once the checkpoint it was freed in has completed. Between those two points the block occupies
disk without holding data, so a write-heavy workload can leave the data files several times larger than
the data in them. Data files are never truncated, so that size is a high-water mark: it does not come back
down on its own. With this option on, the freed blocks are punched out and the space is returned to the
filesystem straight away.

The file *length* is unaffected — only the space the filesystem charges for it. After enabling this, `ls -l`
and `du --apparent-size` keep reporting the high-water mark while `du` and `df` report what is actually
used, and the two can differ by a large factor. Monitoring that reads the apparent size will look wrong.

Requires a filesystem that supports hole punching (`FALLOC_FL_PUNCH_HOLE`); ext4, XFS and Btrfs do. On one
that does not, OrioleDB logs a single warning and carries on without punching. Compressed tables are not
affected by this option, since their space is managed by extent rather than by block.
...
### `orioledb.debug_disable_downlink_fastpath`

|             |     |
| ----------- | --- |
| **Default** | off |

Disable the fastpath downlink search. When on, `find_page()` descends level by level instead of jumping straight to a cached downlink. Useful for tests that need hikeys or downlink stop events.
