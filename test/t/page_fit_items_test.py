...
INDEX_EXTRA_TUPLE_SIZE = 762


class PageFitItemsTest(BaseTest):

	def assertSysTreePagesCount(self, expectedCount, dump = False):
		out = self.node.execute('postgres', "select orioledb_sys_tree_structure(2, 'ne');")[0][0]
		if dump:
			print("\n")
			print(out)
	...
	def extractTupleSize(self):
		...
		tuples = []
		for item in match:
			tuples.append((int(item[0]), int(item[1]), int(item[3]), int(item[-1])))
		return tuples

	def estimateTupleSize(self, relFields, indexCount):
		tupSize = RELATION_SINGLE_FIELD_TUPLE_SIZE + (relFields - 1) * RELATION_EXTRA_FIELD_TUPLE_SIZE + indexCount * INDEX_EXTRA_TUPLE_SIZE
	...
	def test_calibrate_sizes(self):
		...
		for tup in create_tuples:
			self.assertEqual(tup[-1], RELATION_SINGLE_FIELD_TUPLE_SIZE + tup[0] * RELATION_EXTRA_FIELD_TUPLE_SIZE)



		node.safe_psql('postgres', "CREATE INDEX o_table1_t1 ON o_table1 USING BTREE(t1);\n"
		...
		for i, tup in enumerate(with_index_tuples):
			self.assertEqual(tup[-1], create_tuples[i][-1] + INDEX_EXTRA_TUPLE_SIZE)

		node.safe_psql('postgres', "CREATE INDEX o_table2_t2 ON o_table2 USING BTREE(t2);")
		node.safe_psql('postgres', "CREATE INDEX o_table3_t2 ON o_table3 USING BTREE(t2);")
		...
		for i, tup in enumerate(with_index_tuples):
			if i == 0:
				continue
			self.assertEqual(tup[-1], create_tuples[i][-1] + 2 * INDEX_EXTRA_TUPLE_SIZE)
	...
	def test_new_tuple_free_space_enought_fit_as_is(self):
		...
		node.safe_psql('postgres', "CREATE TABLE o_table4(t1 text, t2 text, t3 text, t4 text) USING orioledb;\n")
	...
	def test_new_tuple_sum_free_and_vacated_enought_fit_compact(self):
		...
		node.start()
		node.safe_psql('postgres', "CREATE EXTENSION IF NOT EXISTS orioledb;\n")
		relDescr = self.startup_page_filling(8192 - self.estimateTupleSize(1, 0))
		self.assertSysTreePagesCount(1)
		...
		node.safe_psql('postgres', f"CREATE TABLE o_test{len(relDescr) + 1}(t1 text, t2 text) USING orioledb;")
	...
	def test_new_tuple_free_and_vacated_not_enought_fit_split(self):
		...
		node.safe_psql('postgres', f"CREATE TABLE o_test{len(relDescr) + 1}(t1 text, t2 text, t3 text, t4 text, t5 text, t6 text, t7 text) USING orioledb;")
	...
	def test_replace_decrease_size_no_free_space_fit_as_is(self):
		...
		for i, rel in enumerate(relDescr):
			if rel[1] > 0:
				node.safe_psql('postgres', f"DROP INDEX o_table{i + 1}_idx_t0;\n")
				break
	...
	def test_replace_free_space_enought_fit_as_is(self):
		...
		node.start()
		node.safe_psql('postgres', "CREATE EXTENSION IF NOT EXISTS orioledb;\n")
		relDescr = self.startup_page_filling(8192 - INDEX_EXTRA_TUPLE_SIZE - 150)
		self.assertSysTreePagesCount(1)

		for i, rel in enumerate(relDescr):
			if rel[1] < rel[0]:
				node.safe_psql('postgres', f"CREATE INDEX o_table{i+1}_idx_t{rel[0] - 1} ON o_table{i} USING BTREE(t{rel[0] - 1});\n")
				break
	...
	def test_replace_use_vacated_from_live_tuple_for_another_fit_compact(self):
		...
		for i, rel in enumerate(relDescr):
			if i != vacatedTuple and rel[0] > rel[1]:
				node.safe_psql('postgres', f"CREATE INDEX o_table{i + 1}_t{rel[0] - 1} ON o_table{i + 1} USING BTREE(t{rel[0] - 1})\n;")
				break
	...
	def test_replace_encrease_self_vacated_enought_fit_compact(self):
		...
		node.safe_psql('postgres', f"CREATE INDEX o_table{vacatedTuple + 1}_idx_t0 ON o_table{vacatedTuple + 1} USING BTREE(t0) where t0 > 'abcd';")
	...
	def test_replace_drop_in_first_trasaction_commit(self):
		...
		con1 = node.connect()
		con2 = node.connect()
		t1 = ThreadQueryExecutor(con1, f"DROP TABLE o_table{transactionRel + 1};")
		t2 = ThreadQueryExecutor(con2, f"CREATE INDEX o_table{transactionRel + 1}_idx_t0 ON o_table{transactionRel + 1} USING BTREE(t0)\n;")
		t1.start()
		t1.join()
		self.assertSysTreePagesCount(1)
		t2.start()
		...
		with self.assertRaises(DatabaseError) as e:
			t2.join()
			con2.commit()
		self.assertErrorMessageEquals(e, f'relation "o_table{transactionRel + 1}" does not exist')
	...
	def test_replace_drop_in_second_trasaction_commit(self):
		...
		con1 = node.connect()
		con2 = node.connect()
		t1 = ThreadQueryExecutor(con1, f"CREATE INDEX o_table{transactionRel + 1}_idx_t0 ON o_table{transactionRel + 1} USING BTREE(t0)\n;")
		t2 = ThreadQueryExecutor(con2, f"DROP TABLE o_table{transactionRel + 1};")
		t1.start()
		t1.join()
		self.assertSysTreePagesCount(3)
