...
class CheckpointConcurrentTest(BaseTest):
	...
	def test_checkpoint_rightlink_ok(self):
		...
		node.start()
		self.assertEqual(
		    node.execute("SELECT count(*) FROM o_checkpoint;")[0][0], 6)
		node.stop()
	...
	def test_concurrent_compress_eviction_change_compressed_size(self):
		...
		con1.execute(
		    "INSERT INTO o_evicted (SELECT id, id + 1 FROM generate_series(0, 150000, 5) id);\n"
		)
		con1.commit()
		...
		con1.execute(
		    "INSERT INTO o_evicted (SELECT id, id + 1 FROM generate_series(1, 100000, 5) id);\n"
		)
		con1.commit()
		node.execute("SELECT * FROM o_test;")

		con1.execute(
		    "INSERT INTO o_evicted (SELECT id, id + 1 FROM generate_series(2, 100000, 5) id);\n"
		)
		con1.commit()
		node.execute("SELECT * FROM o_test;")

		con1.execute(
		    "INSERT INTO o_evicted (SELECT id, id + 1 FROM generate_series(3, 100000, 5) id);\n"
		)
		con1.commit()
		node.execute("SELECT * FROM o_test;")

		con1.execute(
		    "INSERT INTO o_evicted (SELECT id, id + 1 FROM generate_series(4, 50000, 5) id);\n"
		)
		con1.commit()
		node.execute("SELECT * FROM o_test;")
	...
	def test_concurrent_compress_create_map_eviction(self):
		node = self.node
		node.append_conf(
		    'postgresql.conf', "orioledb.main_buffers = 16MB\n"
		    "log_min_messages = notice\n"
		    "orioledb.enable_stopevents = true\n")
		node.start()  # start PostgreSQL
		node.safe_psql(
		...
		con1.execute(
		    "INSERT INTO o_evicted1 (SELECT id, id + 1 FROM generate_series(0, 150000, 5) id);\n"
		)
		con1.commit()
		con1.execute(
		    "INSERT INTO o_evicted2 (SELECT id, id + 1 FROM generate_series(0, 150000, 5) id);\n"
		)
		con1.commit()
		...
		con1.execute(
		    "INSERT INTO o_evicted1 (SELECT id, id + 1 FROM generate_series(1, 100000, 5) id);\n"
		)
		con1.commit()
		con1.execute(
		    "INSERT INTO o_evicted2 (SELECT id, id + 1 FROM generate_series(1, 100000, 5) id);\n"
		)
		con1.commit()
		node.execute("SELECT * FROM o_test;")

		con1.execute(
		    "INSERT INTO o_evicted1 (SELECT id, id + 1 FROM generate_series(2, 100000, 5) id);\n"
		)
		con1.commit()
		con1.execute(
		    "INSERT INTO o_evicted2 (SELECT id, id + 1 FROM generate_series(2, 100000, 5) id);\n"
		)
		con1.commit()
		node.execute("SELECT * FROM o_test;")
		...
		con1.execute(
		    "INSERT INTO o_evicted1 (SELECT id, id + 1 FROM generate_series(3, 100000, 5) id);\n"
		)
		con1.commit()
		con1.execute(
		    "INSERT INTO o_evicted2 (SELECT id, id + 1 FROM generate_series(3, 100000, 5) id);\n"
		)
		con1.commit()
		node.execute("SELECT * FROM o_test;")

		con1.execute(
		    "INSERT INTO o_evicted1 (SELECT id, id + 1 FROM generate_series(4, 50000, 5) id);\n"
		)
		con1.commit()
		con1.execute(
		    "INSERT INTO o_evicted2 (SELECT id, id + 1 FROM generate_series(4, 50000, 5) id);\n"
		)
		con1.commit()
		node.execute("SELECT * FROM o_test;")
	...
	def test_checkpoint_concurrent_bruteforce_eviction_load(self):
		...
		for t in range(0, 40):
			node.safe_psql(
			    'postgres', "CREATE TABLE IF NOT EXISTS t%d (\n"
			    "	val text\n"
			    ") USING orioledb;" % t)
			node.safe_psql(
			    'postgres', "INSERT INTO t%d\n"
	...
	def test_checkpoint_autonomous_evict_random(self):
		...
		node.safe_psql('postgres', "CREATE EXTENSION IF NOT EXISTS orioledb;")
		node.safe_psql(
		    'postgres', """CREATE TABLE IF NOT EXISTS o_checkpoint (
										id text NOT NULL,
										PRIMARY KEY (id)
									) USING orioledb;""")
		node.safe_psql(
		    'postgres',
		    "INSERT INTO o_checkpoint (SELECT to_char(id, 'fm0000') || repeat('x', 2500) FROM generate_series(3500,7000,100) id);"
		)
		...
		con3.execute(
		    ...
		    "'$.action == \"walkUpwards\" && "
		    "$.treeName == \"o_checkpoint_pkey\" && "
		    "($.nextKey.value.id > \"%d\" || $.nextKey.type == \"greatest\")');"
		...
		for i in range(3500, 7200, 100):
			con2.execute(
			    "INSERT INTO o_checkpoint (SELECT to_char(id, 'fm0000') || repeat('y', 2500) FROM generate_series(%s, %s) id);"
			    % (str(i), str(i + 99)))
		con2.commit()
