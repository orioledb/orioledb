...
class MergeTest(BaseTest):
	...
	def test_merge_diff_old_snapshot_read(self):
		...
		self.assertEqual(
		    con_snap.execute(
		        "SELECT min(id), max(id), sum(id::bigint) FROM %s;" % table)[0],
		    (1, 3000, 4501500))
		con_snap.rollback()
		con_snap.close()
		...
		self.assertTrue(
		    self.node.execute(
		        "SELECT orioledb_tbl_check('%s'::regclass)" % table)[0][0])
	...
	def test_merge_diff_old_snapshot_index_read(self):
		...
		self.assertEqual(
		    con_snap.execute("SELECT id FROM %s "
		                     "WHERE id BETWEEN 500 AND 2500 ORDER BY id;" % table),
		    [(i, ) for i in range(500, 2501)])
		self.assertEqual(
		    con_snap.execute("SELECT id FROM %s "
		                     "WHERE id BETWEEN 500 AND 2500 ORDER BY id DESC;"
		                     % table), [(i, ) for i in range(2500, 499, -1)])
		self.assertEqual(
		    con_snap.execute(
		        "SELECT min(id), max(id), sum(id::bigint) FROM %s;" % table)[0],
		    (1, 3000, 4501500))
	...
	def test_merge_diff_old_snapshot_point_read(self):
		...
		self.assertEqual(
		    con_snap.execute("SELECT count(*) FROM %s "
		                     "WHERE id IN (700, 1300, 2200, 2900);" % table)[0][0],
		    4)
