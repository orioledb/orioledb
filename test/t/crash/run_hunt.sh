...
TRIALS=${1:-1}
INSTANCE=${2:-bank_hunt}

REPO_ROOT=$(cd "$(dirname "$0")/../../.." && pwd)

# Activate a venv only when VENV_ACTIVATE is explicitly set; otherwise run
# against whatever python3 is already on PATH (no venv). A set-but-missing
# path is still an error, to catch typos.
	...
	fails=$(awk -v trial="$trial" '
		...
		END {
			for (h in plans) {
				...
				# Forward/Backward index scan of: o_bank_account_pkey).
				# pg18 prefers the latter for id-ordered/DISTINCT-id
				# queries since the pkey scan supplies the ordering for
				...
				if (kind == "pk") {
					if (!pk_used)
						print "[plan-check trial=" trial " FAIL] " h \
						      " missing PK scan (Seq Scan or o_bank_account_pkey)"
					if (idx_used)
						print "[plan-check trial=" trial " FAIL] " h \
...
# Fail (non-zero) if any trial was BUG/ERROR/TIMEOUT or a plan-check failed,
# so `make bankcrashcheck` / the bank_account_crash CI check goes red unless
# the whole hunt was clean.
exit $(( n_bug || n_error || n_timeout || plan_fail ))
