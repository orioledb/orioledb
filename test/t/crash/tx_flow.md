...
[finish_xact_command](https://github.com/orioledb/postgres/blob/e43537cdc36146f1becc0084b1acc24a46074ae6/src/backend/tcop/postgres.c#L1303) 
	...
	[CommitTransactionCommand](https://github.com/orioledb/postgres/blob/e43537cdc36146f1becc0084b1acc24a46074ae6/src/backend/tcop/postgres.c#L2805) -> [CommitTransactionCommandInternal](https://github.com/orioledb/postgres/blob/ac88d9a17c6eadfca2e55fc2f18b915283587271/src/backend/access/transam/xact.c#L3136) -> [CommitTransaction](https://github.com/orioledb/postgres/blob/ac88d9a17c6eadfca2e55fc2f18b915283587271/src/backend/access/transam/xact.c#L3175)
		...
		|
		[PreCommit_on_commit_actions](https://github.com/orioledb/postgres/blob/1d7f7407002d8f7ffb2f5c815cef418fa10f3777/src/backend/commands/tablecmds.c#L17638) **good point for injection (`postgres-precommit-on-commit-actions`)** due commets before function explicitly says that error may be ecountered. Function itself truncate and delete some relations/rows that should be removed after tx commit. Function perform a lot modifications, so it seems good to inject faults in multiple places within that func. (e.g. [before/after of pushing snapshot](https://github.com/orioledb/postgres/blob/1d7f7407002d8f7ffb2f5c815cef418fa10f3777/src/backend/commands/tablecmds.c#L17706))
		| 
		...
		|
		**[HOLD_INTERRUPTS at [xact.c#L2319](https://github.com/orioledb/postgres/blob/ac88d9a17c6eadfca2e55fc2f18b915283587271/src/backend/access/transam/xact.c#L2319)]** -- defers signal-driven cancel/die from here. `SIGTERM` / `SIGINT` / `SIGALRM` set their pending flag but the next `CHECK_FOR_INTERRUPTS` is a no-op until `RESUME_INTERRUPTS` below. **Inside this scope, only code-level `ereport`/`Assert`/`palloc OOM` can raise** -- async signals cannot trigger `ERROR/FATAL`.
		|
		[RecordTransactionCommit](https://github.com/orioledb/postgres/blob/ac88d9a17c6eadfca2e55fc2f18b915283587271/src/backend/access/transam/xact.c#L2343) -- actual machinery for marking tx as commited. **good point for injection** 
			|
			**[START_CRIT_SECTION at [xact.c#L1439](https://github.com/orioledb/postgres/blob/ac88d9a17c6eadfca2e55fc2f18b915283587271/src/backend/access/transam/xact.c#L1439)]** -- nested inside the outer `HOLD_INTERRUPTS`. **Any `ereport(ERROR)` raised inside this scope escalates to PANIC** (crit-sections are nestable; the outer holdoff is already active so the only practical addition is the PANIC escalation).
			|
			...
			|
			**[END_CRIT_SECTION at [xact.c#L1544](https://github.com/orioledb/postgres/blob/ac88d9a17c6eadfca2e55fc2f18b915283587271/src/backend/access/transam/xact.c#L1544)]** -- WAL commit record durably written; outer `HOLD_INTERRUPTS` still in effect.
		|
			...
			[undo_xact_callback](https://github.com/orioledb/orioledb/blob/187f850a6ec182ce80f31c9c1594ac0dc26fe0b8/src/transam/undo.c#L2110) with event = XACT_EVENT_COMMIT 
				...
				[assign_xidless_commit_lsn](https://github.com/orioledb/orioledb/blob/187f850a6ec182ce80f31c9c1594ac0dc26fe0b8/src/transam/undo.c#L2262) 
					...
					[wal_commit](https://github.com/orioledb/orioledb/blob/187f850a6ec182ce80f31c9c1594ac0dc26fe0b8/src/transam/undo.c#L220) 
						...
						|
						[add_finish_wal_record](https://github.com/orioledb/orioledb/blob/6f900ca6cf4b8eee3eec0634254ec21ee8c8918d/src/recovery/wal.c#L296) -- **good point (`orioledb-add-finish-wal` / `orioledb-add-finish-wal-guarded`)** It worths testing to inject-fault during wal building right before finish record. **RECURSIVE -- raises PANIC if injected at the function level without an `isCommit` guard.** `add_finish_wal_record` is also called on the abort path from `wal_rollback` at [wal.c#L358](https://github.com/orioledb/orioledb/blob/6f900ca6cf4b8eee3eec0634254ec21ee8c8918d/src/recovery/wal.c#L358) (`add_finish_wal_record(WAL_REC_ROLLBACK, ...)`). An `error`-mode injection inside the function entry would fire on both `WAL_REC_COMMIT` (commit) and `WAL_REC_ROLLBACK` (abort); the second hit happens during `XACT_EVENT_ABORT` -> PANIC. Mitigation: place the `INJECTION_POINT` either at the call site in `wal_commit` only ([wal.c#L296](https://github.com/orioledb/orioledb/blob/6f900ca6cf4b8eee3eec0634254ec21ee8c8918d/src/recovery/wal.c#L296)), or branch on the `rec_type` parameter inside the function (`if (rec_type == WAL_REC_COMMIT) INJECTION_POINT(...)`), or use distinct point names for commit vs rollback finishes.
						|
						[INJECTION_POINT at wal.c#L336](https://github.com/orioledb/orioledb/blob/HEAD/src/recovery/wal.c#L336) -- **good point for injection (`orioledb-before-pre-commit-wal-finish`)** PANIC-escalating, wrapped in `START_CRIT_SECTION` / `END_CRIT_SECTION` so an attached `error` action escalates to PANIC instead of a clean abort. Fires *between* the call site of `add_finish_wal_record` above and the actual append of `WAL_REC_COMMIT`, while the modify records (already buffered, possibly already flushed by the earlier `flush_local_wal_if_needed`) sit on disk *without* a matching commit-or-rollback marker. Distinct from `orioledb-add-finish-wal` (clean abort, no commit marker emitted but the abort handler emits `WAL_REC_ROLLBACK` so recovery sees a clear rollback); here the postmaster crashes between the modify-record flush and any finish marker, so recovery sees orphan modify records with NO `WAL_REC_COMMIT` and NO `WAL_REC_ROLLBACK` -- the exact scenario where recovery must infer "no marker -> tx was in-flight -> discard" purely from the absence of a finish record. Used only by `assert_chaos_loop` (not `wal_chaos_loop`) because attach is guaranteed to PANIC. Commit-side only -- not reached on the abort path, no `-guarded` companion needed.
						|
						`wal_commit` calls [flush_local_wal](https://github.com/orioledb/orioledb/blob/HEAD/src/recovery/wal.c#L343) -- **Layer B**. Drains the entire `local_wal_buffer` for this backend into PG's WAL:
							|
							`flush_local_wal` calls [log_logical_wal_container](https://github.com/orioledb/orioledb/blob/HEAD/src/recovery/wal.c#L805)
								|
								`log_logical_wal_container` does XLogBeginInsert -> XLogRegisterData -> [XLogInsert(ORIOLEDB_RMGR_ID, ORIOLEDB_XLOG_CONTAINER)](https://github.com/orioledb/orioledb/blob/HEAD/src/recovery/wal.c#L911) -- packs the buffer into one ORIOLEDB_XLOG_CONTAINER record, returns its LSN. Bytes are inside PG's WAL buffer; **not yet fsync'd** -- durability comes with `XLogFlush(flushPos)` below.
					|
					[XLogFlush(flushPos)](https://github.com/orioledb/orioledb/blob/187f850a6ec182ce80f31c9c1594ac0dc26fe0b8/src/transam/undo.c#L2272) -- **Layer C: the orioledb-commit durability barrier**. fsyncs PG WAL up to `flushPos` (the LSN returned by `flush_local_wal`). Gated by `if (synchronous_commit > SYNCHRONOUS_COMMIT_OFF || oxid_needs_wal_flush)` -- so async-commit mode skips it and durability is left to the next WAL writer cycle / checkpoint. **This is *the* sync point a writer's `con.commit()` blocks on for orioledb-only txs.** The commit-window cascade race we documented earlier sits in the interval between `XLogFlush` returning here and the Python `my_token = to_token` assignment landing in the writer's local state.
					|
					(possibly: [SyncRepWaitForLSN](https://github.com/orioledb/orioledb/blob/187f850a6ec182ce80f31c9c1594ac0dc26fe0b8/src/transam/undo.c#L2276) -- block on synchronous replicas if `synchronous_standby_names` is set; off by default in the test)
					| 
				...
				| 
				[INJECTION_POINT at undo.c#L2378](https://github.com/orioledb/orioledb/blob/HEAD/src/transam/undo.c#L2378) -- **good point for injection (`orioledb-commit-assert`)** PANIC-escalating, wrapped in `START_CRIT_SECTION` / `END_CRIT_SECTION` so an attached `error` action escalates to PANIC. Fires *after* the WAL pipeline above has appended **and flushed** `WAL_REC_COMMIT` (the commit record is durable on disk), but *before* `current_oxid_precommit` flips the in-memory CSN to `COMMITTING`. Distinct recovery scenario from `orioledb-before-pre-commit-wal-finish`: here recovery sees a fully-committed WAL stream for the crashed oxid but the cluster never observed the in-memory commit, so replay re-applies the commit through the normal `WAL_REC_COMMIT` path. Used only by `assert_chaos_loop` (not `wal_chaos_loop`) because attach is guaranteed to PANIC. Bracketed by `commit-assert-trace pre`/`post` LOG lines so post-test analysis can count per-pid catches. Commit-side only -- not reached on the abort path, no `-guarded` companion needed.
				|
		...
		|
		**[RESUME_INTERRUPTS at [xact.c#L2484](https://github.com/orioledb/postgres/blob/ac88d9a17c6eadfca2e55fc2f18b915283587271/src/backend/access/transam/xact.c#L2484)]** -- pending signal flags raised during the holdoff are processed at the next `CHECK_FOR_INTERRUPTS` after this point.
|
...
* **(a) inline eviction on page-miss** -- if the page the read needs isn't in the pool and the pool is full, the loader picks a victim before installing the new page:
	|
	`o_btree_find_tuple_by_key_cb` calls [find_page](https://github.com/orioledb/orioledb/blob/HEAD/src/btree/iterator.c#L155) -- B-tree descent
		|
		`find_page` reaches a page-pool miss and calls [load_page](https://github.com/orioledb/orioledb/blob/HEAD/src/btree/find.c#L797)
			|
			when the page-pool is full, `ppool_run_maintenance` (called from the page-pool allocator) iterates [walk_page](https://github.com/orioledb/orioledb/blob/HEAD/src/utils/page_pool.c#L437)
				|
				when the victim is dirty, [walk_page calls write_page](https://github.com/orioledb/orioledb/blob/HEAD/src/btree/io.c#L3083)
					|
					**Layer B** (the actual write): `write_page` invokes [perform_page_io](https://github.com/orioledb/orioledb/blob/HEAD/src/btree/io.c#L2249) -> [write_page_to_disk](https://github.com/orioledb/orioledb/blob/HEAD/src/btree/io.c#L1957) -> [btree_smgr_write](https://github.com/orioledb/orioledb/blob/HEAD/src/btree/io.c#L1432) -> [OFileWrite](https://github.com/orioledb/orioledb/blob/HEAD/src/btree/io.c#L589) -> `FileWrite` (PG VFD)
					|
					**Layer A** (kernel-writeback scheduling, batched): `write_page` calls [writeback_put_extent](https://github.com/orioledb/orioledb/blob/HEAD/src/btree/io.c#L2253) into `io_writeback`; when `extentsNumber >= bgwriter_flush_after`, [perform_writeback](https://github.com/orioledb/orioledb/blob/HEAD/src/btree/io.c#L2297) flushes via direct [FileWriteback](https://github.com/orioledb/orioledb/blob/HEAD/src/btree/io.c#L3380) (or [msync(MS_ASYNC)](https://github.com/orioledb/orioledb/blob/HEAD/src/btree/io.c#L3374) for `use_mmap` storage)
												...
												[o_btree_modify](https://github.com/orioledb/orioledb/blob/187f850a6ec182ce80f31c9c1594ac0dc26fe0b8/src/tableam/operations.c#L1578-L1582) with action = `BTreeOperationUpdate` -- public B-tree modify entry. Just dispatches to `o_btree_normal_modify`.
													...
													[o_btree_normal_modify](https://github.com/orioledb/orioledb/blob/187f850a6ec182ce80f31c9c1594ac0dc26fe0b8/src/btree/modify.c#L987) -- reserves undo space, reserves page-pool capacity, then walks the tree. STOPEVENT `modify_start` fires at the entry. Calls [find_page](https://github.com/orioledb/orioledb/blob/187f850a6ec182ce80f31c9c1594ac0dc26fe0b8/src/btree/iterator.c#L155) (or `refind_page` when the caller passed a hint) to locate the target leaf and acquire the page write-lock; on success dispatches into `o_btree_modify_internal` with the populated page-find context.
														...
														[o_btree_modify_internal](https://github.com/orioledb/orioledb/blob/187f850a6ec182ce80f31c9c1594ac0dc26fe0b8/src/btree/modify.c#L110) -- operates on the already-found, write-locked leaf page. Reads the existing tuple via `BTREE_PAGE_READ_LEAF_ITEM`, runs the modify-callback (`o_update_callback`) for visibility / row-lock / self-modification decisions, and dispatches by action: `o_btree_modify_delete` / `o_btree_modify_lock` for those, or `o_btree_modify_insert_update` for the insert/update branch.
															...
															[o_btree_modify_handle_conflicts](https://github.com/orioledb/orioledb/blob/187f850a6ec182ce80f31c9c1594ac0dc26fe0b8/src/btree/modify.c#L428) -- visibility / row-lock resolution against the existing leaf tuple. Holds the leaf write-lock on entry. When the conflicting tuple belongs to a foreign oxid, calls [oxid_get_csn](https://github.com/orioledb/orioledb/blob/187f850a6ec182ce80f31c9c1594ac0dc26fe0b8/src/btree/modify.c#L487) **while still holding the page lock** to classify it. Returns one of `ConflictResolutionOK` (caller falls through to dispatch), `ConflictResolutionFound` (waitCallback said Exit), or `ConflictResolutionRetry` (caller `goto retry`s the leaf-read).
																|  -- on `ABORTED`: roll the foreign tuple back inline via **[START_CRIT_SECTION at [modify.c#L543](https://github.com/orioledb/orioledb/blob/187f850a6ec182ce80f31c9c1594ac0dc26fe0b8/src/btree/modify.c#L543)]** -> `page_item_rollback` -> **[END_CRIT_SECTION at [modify.c#L550](https://github.com/orioledb/orioledb/blob/187f850a6ec182ce80f31c9c1594ac0dc26fe0b8/src/btree/modify.c#L550)]**, then continue; on `NORMAL` / `FROZEN`: raise `ERRCODE_T_R_SERIALIZATION_FAILURE` if isolation is RR / SERIALIZABLE and `csn >= context->opCsn`; on `INPROGRESS`: take the wait sub-flow below
																**Wait sub-flow** (`COMMITSEQNO_IS_INPROGRESS` branch) -- asks `waitCallback` (when set) for Wait / NoWait / Exit, then [unlock_page(blkno)](https://github.com/orioledb/orioledb/blob/187f850a6ec182ce80f31c9c1594ac0dc26fe0b8/src/btree/modify.c#L558) drops the leaf write-lock before sleeping.
																|  -- on `OBTreeCallbackActionXidWait` (Exit short-circuits to `ConflictResolutionFound`; NoWait skips the sleep entirely)
																[wait_for_tuple](https://github.com/orioledb/orioledb/blob/187f850a6ec182ce80f31c9c1594ac0dc26fe0b8/src/btree/modify.c#L400) at [modify.c#L563](https://github.com/orioledb/orioledb/blob/187f850a6ec182ce80f31c9c1594ac0dc26fe0b8/src/btree/modify.c#L563) -- acquires a heavyweight tuple lock via `LockAcquire(SET_LOCKTAG_TUPLE(...))`, then [wait_for_oxid](https://github.com/orioledb/orioledb/blob/187f850a6ec182ce80f31c9c1594ac0dc26fe0b8/src/transam/oxid.c) parks on PG's xact-completion infrastructure until the foreign oxid commits or aborts.
																|  -- after wake (foreign oxid has now committed or aborted)
																[refind_page](https://github.com/orioledb/orioledb/blob/187f850a6ec182ce80f31c9c1594ac0dc26fe0b8/src/btree/find.c#L986) at [modify.c#L575](https://github.com/orioledb/orioledb/blob/187f850a6ec182ce80f31c9c1594ac0dc26fe0b8/src/btree/modify.c#L575) -- the leaf may have split / merged while we slept, so re-locate it via the saved `blkno` + `pageChangeCount` hint and re-acquire the write-lock through [lock_page](https://github.com/orioledb/orioledb/blob/187f850a6ec182ce80f31c9c1594ac0dc26fe0b8/src/btree/page_state.c#L403) (which sleeps on `PGSemaphoreLock(MyProc->sem)` if another backend is in the page's `lockerStates` queue ahead of us). Returns `ConflictResolutionRetry`; back in `o_btree_modify_internal` the `goto retry` re-runs `find_page` + leaf-read + `handle_conflicts`.
															|  -- on `ConflictResolutionOK` (or when no matching key existed in the first place, in which case `handle_conflicts` was skipped entirely)
...
* **(b) the next checkpoint** -- PG calls orioledb's [CheckPoint_hook -> o_perform_checkpoint](https://github.com/orioledb/orioledb/blob/HEAD/src/orioledb.c#L1259) (registered at extension load):
	|
	`o_perform_checkpoint` calls [o_indices_foreach_oids](https://github.com/orioledb/orioledb/blob/HEAD/src/checkpoint/checkpoint.c#L1414) with `checkpoint_tables_callback`
		|
		per table the callback reaches [checkpoint_btree](https://github.com/orioledb/orioledb/blob/HEAD/src/checkpoint/checkpoint.c#L1749) -> [checkpoint_btree_loop](https://github.com/orioledb/orioledb/blob/HEAD/src/checkpoint/checkpoint.c#L3031) -- depth-first post-order walk
			|
			on visiting the dirty leaf, [`checkpoint_btree_loop` calls perform_page_io](https://github.com/orioledb/orioledb/blob/HEAD/src/checkpoint/checkpoint.c#L3769) -> [write_page_to_disk](https://github.com/orioledb/orioledb/blob/HEAD/src/btree/io.c#L1957) (**Layer B**, same sink as eviction)
			|
			then [writeback_put_extent](https://github.com/orioledb/orioledb/blob/HEAD/src/checkpoint/checkpoint.c#L3789) into the per-subtree `CheckpointWriteBack`; when full, `checkpoint_btree_loop` calls [perform_writeback_and_relock](https://github.com/orioledb/orioledb/blob/HEAD/src/checkpoint/checkpoint.c#L3638) -> [perform_writeback](https://github.com/orioledb/orioledb/blob/HEAD/src/checkpoint/checkpoint.c#L561) -> [btree_smgr_writeback](https://github.com/orioledb/orioledb/blob/HEAD/src/checkpoint/checkpoint.c#L453) -> [FileWriteback](https://github.com/orioledb/orioledb/blob/HEAD/src/btree/io.c#L742) (**Layer A**)
			|
			at end of checkpoint, segment-file durability via [btree_smgr_sync](https://github.com/orioledb/orioledb/blob/HEAD/src/checkpoint/checkpoint.c#L2732) -> `FileSync` per segment file -- **Layer C** durability barrier for everything just written.
...
**The WAL record is now in the per-backend `local_wal_buffer` (8 KB, in shared memory).** The buffer drains to PG's WAL only on overflow or at commit. The overflow path is:
	|
	`add_modify_wal_record_extended` calls [flush_local_wal_if_needed](https://github.com/orioledb/orioledb/blob/HEAD/src/recovery/wal.c#L187) at the top -- fires only when `offset + required + XID_RESERVED > LOCAL_WAL_BUFFER_SIZE`
		|
		`flush_local_wal_if_needed` calls [flush_local_wal](https://github.com/orioledb/orioledb/blob/HEAD/src/recovery/wal.c#L852) on overflow -- drains the entire local buffer
			|
			`flush_local_wal` calls [log_logical_wal_container](https://github.com/orioledb/orioledb/blob/HEAD/src/recovery/wal.c#L805)
				|
				`log_logical_wal_container` does XLogBeginInsert -> XLogRegisterData (version, flags, xact info, origin, payload) -> [XLogInsert(ORIOLEDB_RMGR_ID, ORIOLEDB_XLOG_CONTAINER)](https://github.com/orioledb/orioledb/blob/HEAD/src/recovery/wal.c#L911) -- **Layer B** (one ORIOLEDB_XLOG_CONTAINER record per drain; bytes land in PG WAL buffer, not yet fsync'd)
	...
	[AbortCurrentTransaction](https://github.com/orioledb/postgres/blob/e43537cdc36146f1becc0084b1acc24a46074ae6/src/backend/tcop/postgres.c#L4482) -- the wrapper that drives the abort regardless of which TBLOCK state we were in.
		...
		[AbortCurrentTransactionInternal](https://github.com/orioledb/postgres/blob/ac88d9a17c6eadfca2e55fc2f18b915283587271/src/backend/access/transam/xact.c#L3430) -- state-machine dispatch; for the in-block-tx case (`TBLOCK_INPROGRESS`) unconditionally calls `AbortTransaction`.
			...
			[AbortTransaction](https://github.com/orioledb/postgres/blob/ac88d9a17c6eadfca2e55fc2f18b915283587271/src/backend/access/transam/xact.c#L3464) -- the actual rollback machinery. Fires xact-event callbacks, releases resources, transitions oxid state.
				...
				|
				**[HOLD_INTERRUPTS at [xact.c#L2790](https://github.com/orioledb/postgres/blob/ac88d9a17c6eadfca2e55fc2f18b915283587271/src/backend/access/transam/xact.c#L2790)]** -- same shape as the commit-side holdoff; signals are deferred for the entire abort body.
				|
				[RecordTransactionAbort](https://github.com/orioledb/postgres/blob/ac88d9a17c6eadfca2e55fc2f18b915283587271/src/backend/access/transam/xact.c#L2906) -- writes `XLOG_XACT_ABORT` to PG's xlog (no-op for the bank-test writer because the local-WAL has only orioledb records, not heap records). Internally wraps the WAL emission in **[START_CRIT_SECTION at [xact.c#L1802](https://github.com/orioledb/postgres/blob/ac88d9a17c6eadfca2e55fc2f18b915283587271/src/backend/access/transam/xact.c#L1802)]** -> XLogInsert -> **[END_CRIT_SECTION at [xact.c#L1846](https://github.com/orioledb/postgres/blob/ac88d9a17c6eadfca2e55fc2f18b915283587271/src/backend/access/transam/xact.c#L1846)]**.
				|
					...
					[undo_xact_callback](https://github.com/orioledb/orioledb/blob/187f850a6ec182ce80f31c9c1594ac0dc26fe0b8/src/transam/undo.c#L2110) with event = XACT_EVENT_ABORT -- the function's body runs the steps below in order. The XACT_EVENT_ABORT switch case at [undo.c:2327](https://github.com/orioledb/orioledb/blob/187f850a6ec182ce80f31c9c1594ac0dc26fe0b8/src/transam/undo.c#L2327) is reached only after the pre-switch setup completes.
						...
						**XACT_EVENT_ABORT switch case** -- the actual abort work begins below.
						...
						[wal_rollback](https://github.com/orioledb/orioledb/blob/187f850a6ec182ce80f31c9c1594ac0dc26fe0b8/src/transam/undo.c#L2334) -- emits `WAL_REC_ROLLBACK` if the tx had material changes. **No-op for read-only or no-change txs.** The early bail at `local_wal_has_material_changes == false` skips WAL emission entirely.
							...
							|
							`wal_rollback` calls [flush_local_wal](https://github.com/orioledb/orioledb/blob/HEAD/src/recovery/wal.c#L418) -- **Layer B**. Same chain as the commit-side drain: [log_logical_wal_container at wal.c#L805](https://github.com/orioledb/orioledb/blob/HEAD/src/recovery/wal.c#L805) -> `XLogBeginInsert` + `XLogRegisterData` + [XLogInsert(ORIOLEDB_RMGR_ID, ORIOLEDB_XLOG_CONTAINER)](https://github.com/orioledb/orioledb/blob/HEAD/src/recovery/wal.c#L911). Difference is the finish marker is `WAL_REC_ROLLBACK`, not `WAL_REC_COMMIT`.
								|
								`wal_rollback` then calls [XLogFlush(wait_pos)](https://github.com/orioledb/orioledb/blob/HEAD/src/recovery/wal.c#L428) -- **Layer C**, conditional on `synchronous_commit > SYNCHRONOUS_COMMIT_OFF`. Default async-commit setting skips it; the rollback record stays in PG's WAL buffer and becomes durable via the next WAL writer cycle.

							**NOT suitable for re-injection here** -- we are already in the abort path; raising again would re-enter abort and PANIC. The `wal_chaos` injection point is gated by `isCommit` to skip exactly this call.
						|
			...
			|
			**[RESUME_INTERRUPTS at [xact.c#L2973](https://github.com/orioledb/postgres/blob/ac88d9a17c6eadfca2e55fc2f18b915283587271/src/backend/access/transam/xact.c#L2973)]** -- holdoff released; pending signal flags from the abort body now process on the next `CHECK_FOR_INTERRUPTS`.
			|
...
### `assert_chaos_loop` -- guaranteed-PANIC injections for crash-recovery testing
...
**direct synchronous writes** to a file -- `OFileWrite` / `pg_pwrite` --
...
- **COMMIT path** (the canonical sequence):
  ...
  2. **`XLogFlush(flushPos)`** at `undo.c:2272` -- **Layer C durability barrier** for the commit record.
...
- **ABORT path**:
  ...
  2. `XLogFlush` at `wal.c:428` -- **Layer C** when `synchronous_commit > OFF`.
