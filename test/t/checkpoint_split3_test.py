...
class CheckpointSplit3Test(CheckpointSplitBaseTest):
	...
	def test_checkpoint_split_no_deadlock(self):
		...
		con2.execute(
		    "INSERT INTO o_checkpoint (SELECT to_char(id, 'fm0000') || repeat('x', 2500) FROM generate_series(90, 95, 1) id);"
		)
		con2.commit()

		con2.execute(
		    "INSERT INTO o_checkpoint (SELECT '0330' || repeat('x', 2500));")
	...
	def test_checkpoint_split_ok(self):
		...
		node.start()
		self.assertEqual(
		    node.execute("SELECT count(*) FROM o_checkpoint;")[0][0], 10)
		node.stop()
