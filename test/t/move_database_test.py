...
class TablespaceTest(BaseTest):
	...
	def test_replication_drop_tablespace(self):
		...
		with self.getReplica(ts_mapping).start() as replica:
			...
			master.safe_psql("testdb2", """CREATE EXTENSION orioledb;
			...
			ts1_oid = master.execute("select oid from pg_tablespace where spcname = 'user_ts1'")[0][0]
			ts2_oid = master.execute("select oid from pg_tablespace where spcname = 'user_ts2'")[0][0]
			master.safe_psql("testdb", """drop table test_cascade_parts cascade ;""")
			with self.assertRaises(QueryException): 
			    master.safe_psql("DROP TABLESPACE IF EXISTS user_ts1;")
			master.safe_psql("testdb", """drop table another_test_tbl cascade ;""")
			master.safe_psql("testdb2", """drop table another_test_tbl cascade ;""")
			master.safe_psql("testdb2", """drop table test_cascade_parts cascade ;""")
			self.catchup_orioledb(replica)
			master.safe_psql("DROP TABLESPACE IF EXISTS user_ts1;")
			self.catchup_orioledb(replica)
			...
			master.stop()
			replica.stop()
			self.assertFalse(os.path.exists(os.path.join(master.data_dir, "pg_tblspc", str(ts1_oid))))
			self.assertFalse(os.path.exists(os.path.join(replica.data_dir, "pg_tblspc", str(ts1_oid))))
			self.assertTrue(os.path.exists(os.path.join(master.data_dir, "pg_tblspc", str(ts2_oid))))
			self.assertTrue(os.path.exists(os.path.join(master.data_dir, "pg_tblspc", str(ts2_oid))))

			master.start()
			replica.start()

			self.assertEqual(1, master.execute("select count(*) from pg_tablespace where spcname in ('user_ts1', 'user_ts2');")[0][0])
			self.assertEqual(1, replica.execute("select count(*) from pg_tablespace where spcname in ('user_ts1', 'user_ts2');")[0][0])
			master.stop()
			replica.stop()
	...
	def test_replication_drop_inplace_tablespace(self):
		...
		master.append_conf("postgresql.conf", "allow_in_place_tablespaces = true")
		master.start()
		...
		with self.getReplica(ts_mapping).start() as replica:
			...
			ts_oid = master.execute("select oid from pg_tablespace where spcname = 'user_ts'")[0][0]
			ts1_oid = master.execute("select oid from pg_tablespace where spcname = 'user_ts1'")[0][0]
			master.safe_psql("testdb", """drop table test_cascade_parts cascade ;""")
			with self.assertRaises(QueryException): 
			...
			master.stop()
			replica.stop()
			self.assertFalse(os.path.exists(os.path.join(master.data_dir, "pg_tblspc", str(ts_oid))))
			self.assertFalse(os.path.exists(os.path.join(replica.data_dir, "pg_tblspc", str(ts_oid))))
			self.assertTrue(os.path.exists(os.path.join(master.data_dir, "pg_tblspc", str(ts1_oid))))
			self.assertTrue(os.path.exists(os.path.join(master.data_dir, "pg_tblspc", str(ts1_oid))))

			master.append_conf("postgresql.conf", "allow_in_place_tablespaces = false")
			replica.append_conf("postgresql.conf", "allow_in_place_tablespaces = false")
			master.start()
			replica.start()

			self.assertEqual(2, master.execute("select count(*) from pg_tablespace where spcname in ('user_ts', 'user_ts1', 'user_ts2');")[0][0])
			self.assertEqual(2, replica.execute("select count(*) from pg_tablespace where spcname in ('user_ts','user_ts1', 'user_ts2');")[0][0])
			master.stop()
			replica.stop()
