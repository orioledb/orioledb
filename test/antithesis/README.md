...
- `sk-rebuild-desync-client:<sha>` - client workload
    - targets `recovery-sk-rebuild-desync` (see `antithesis/scratchbook/properties/recovery-sk-rebuild-desync.md`): a secondary-index/PK divergence caused by crash-recovery's replay of *other*, unrelated, cleanly-committing transactions, not the crashing one. No `pg_stopevent_set` involved — reproduces via a crash landing near any ordinary commit, so it just keeps committing single-statement INSERT/UPDATE(token)/DELETE operations against a unique-secondary-indexed table (swarmed action mix and batch size per timeline) and lets Antithesis's own fault injection do the rest. Also swarms checkpoint timing per timeline (folded in from the retired sk-recovery-race-chaos workload below) so some timelines additionally bias toward faults landing near a checkpoint boundary, with a `sometimes()` check confirming that overlap actually happens. Reports via Antithesis SDK assertions (`always`/`sometimes`/`reachable`), checked continuously (`anytime_`) and once more after faults stop (`finally_`).
- flake1 - TODO
