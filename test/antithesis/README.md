...
- `sk-rebuild-desync-client:<sha>` - client workload
    - targets `recovery-sk-rebuild-desync` (see `antithesis/scratchbook/properties/recovery-sk-rebuild-desync.md`): a secondary-index/PK divergence caused by crash-recovery's replay of *other*, unrelated, cleanly-committing transactions, not the crashing one.
- `checkpoint-corrupted-tree-silent-skip-client:<sha>` - client workload
    - targets `checkpoint-corrupted-tree-silent-skip` (see `antithesis/scratchbook/properties/checkpoint-corrupted-tree-silent-skip.md`). The property as originally cataloged (a checkpoint silently dropping a corrupted tree from its own bookkeeping while still reporting success) is already fixed at this commit — `evictable_tree_init_meta()` now FATALs on a root-page load failure. This workload targets the open question the fix itself raises instead: does a persistently corrupted, never-repaired tree cause an unbounded FATAL/restart crash loop that never settles?
- flake1 - TODO
- `non-modify-wal-record-replay-safety` - TODO
- `disk-write-enospc-fails-safe` - TODO
- `backup-window-crash-untested` - TODO
- `replay-until-lsn-catalog-desync` - TODO
...
| [workload/sk-rebuild-desync](./config/workload/sk-rebuild-desync)  | adds a client that commits ordinary concurrent DML (no stopevents, swarmed checkpoint timing) and checks PK/SK consistency continuously, targeting recovery's replay of unrelated committed transactions  |
| [workload/checkpoint-corrupted-tree-silent-skip](./config/workload/checkpoint-corrupted-tree-silent-skip)  | adds a client, sharing a PGDATA volume with `orioledb`, that corrupts one table's on-disk root page and repeatedly re-triggers checkpoints against it, checking that the instance doesn't crash-loop indefinitely  |
| flake repro  | todo  |
...
sk-recovery-race-chaos workload used to target (see Components above) --
checkpoint timing is swarmed per timeline rather than fixed.
