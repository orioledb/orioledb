...
| **Implementation status** | **Implemented** — `test/antithesis/sk-rebuild-desync/` (client `sk-rebuild-desync-client`, config `config/workload/sk-rebuild-desync`). `first_sk-rebuild-desync-setup` creates the schema and swarms per-timeline action-weight/batch-size/checkpoint-timing bias (checkpoint config applied dynamically via `ALTER SYSTEM SET` + `pg_reload_conf()`, not a static compose-mounted `postgres.conf`); `parallel_driver_sk-rebuild-desync-dml` commits ordinary single-statement INSERT/UPDATE(token)/DELETE operations with no stopevent pinning; `anytime_sk-rebuild-desync-check` and `finally_sk-rebuild-desync-check` both call the shared `always()` consistency check (PK-row-count == distinct-SK-token-count + `orioledb_tbl_check()`), continuously and once more after faults stop. A `reachable()` marker confirms the DML driver is actually observing fault-induced connection loss mid-transaction, and a `sometimes()` marker (folded in from the now-retired `sk-recovery-race-chaos` workload, which this one superseded and replaced) confirms checkpoints actually overlap DML on timelines that swarmed an aggressive checkpoint-timing preset. Not yet run through `snouty validate` with built images — see the workload's own notes for that follow-up. |
