...
if [ $GITHUB_JOB = "run-benchmark" ]; then
	pip_packages="psycopg2-binary six testgres==1.11.0 python-telegram-bot matplotlib"
elif [ $GITHUB_JOB = "pgindent" ]; then
	pip_packages="psycopg2 six testgres==1.11.0 moto[s3] flask flask_cors boto3 pyOpenSSL yapf"
else
	pip_packages="psycopg2 six testgres==1.11.0 moto[s3] flask flask_cors boto3 pyOpenSSL"
fi
