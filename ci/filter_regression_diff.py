	...
	while equal and len(src_stack) > 0 and len(target_stack) > 0:
		...
		if target_cur[1].startswith("Result"):
			...
			if target_cur[1].startswith('Custom Scan'):
				...
				# allvisfrac=1 may make IOS cheaper than bitmap heap scan
				src_up = True
				target_up = True
			elif target_cur[1].startswith('Index Scan'):
				# allvisfrac=1 may make index scan cheaper than bitmap heap scan
				src_up = True
				target_up = True
			else:
				equal = False
