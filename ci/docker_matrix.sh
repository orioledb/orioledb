...
The regression mode is enabled by default, 
  and it will run the OrioleDB regression tests during the Docker image build.
Be careful, running all combinations will take a long time (more than 24 hours).
...
Options:
  ...
  --compiler TYPE     Specify compiler type
                      ...
                      Default: $COMPILER
  --regression BOOL   Run OrioleDB regression tests during the Docker image build. 
                      Valid options: [ all yes no ]
                      Default: $REGRESSION
  --debug BOOL        Enable debug mode and preserve the build environments. 
                      In this case, each image size exceeds 1-2GB, so it is not recommended to run it in matrix mode.
                      If regression is set to "yes", this option will run the regression tests with " .. || true ", 
                        meaning it will not stop on errors. 
...
Examples:
  ./ci/docker_matrix.sh --base all-dev --pg-major 16 --compiler clang
  ./ci/docker_matrix.sh --base alpine:3.20 --pg-major 16 --compiler gcc --regression no --debug true
  ./ci/docker_matrix.sh --base ubuntu:oracular --pg-major 16 --compiler all --regression yes --debug false

Default behavior:
  ./ci/docker_matrix.sh --base $BASE_MATRIX --pg-major $PG_MAJOR --compiler $COMPILER --regression $REGRESSION --debug $DEBUG

Known problems:
  - ubuntu:20.04 + "--regression yes" "--debug false" is not working due to old version of python.
    ...
    for pg_major in "${pg_major_list[@]}"; do
        for compiler in "${compiler_list[@]}"; do
            for base in "${base_list[@]}"; do
                for regression in $([[ $REGRESSION == "all" ]] && echo "yes no" || echo "$REGRESSION"); do
                    for debug in $([[ $DEBUG == "all" ]] && echo "true false" || echo "$DEBUG"); do

                        # Warning specific combination
                        if [ "$base" = "ubuntu:20.04" ] && [ "$regression" = "yes" ] && [ "$debug" = "false" ]; then
                            echo
                            echo "#!!!!!!!!!!!!"
                            echo "#WARNIG: ubuntu:20.04 regression=yes with DEBUG_MODE=false is not working"
                            echo "#!!!!!!!!!!!!"
                            echo
                        fi
                        ...
                        local docker_tag="${pg_major}-${compiler}-${base_os}-${base_tag}-regression-${regression}-debug-${debug}"
