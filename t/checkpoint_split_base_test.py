...
class CheckpointSplitBaseTest(BaseTest):
	...
	def checkpoint_concurrent_split_base(self,
	                                     init_values,
	                                     breakpoint_event,
	                                     breakpoint_value,
	                                     breakpoint_level,
	                                     values_after_breakpoint,
	                                     rows_amount,
	                                     with_eviction=False,
	                                     select_after_eviction=False,
	                                     with_second_eviction=False,
	                                     with_second_checkpoint=True):
		node = self.node
		node.append_conf(
		...
		node.safe_psql('postgres',
		               "INSERT INTO o_checkpoint %s;" % init_values)
		...
		con3.execute("SELECT pg_stopevent_set('checkpoint_step', '%s');" %
