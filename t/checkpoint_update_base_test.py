...
class CheckpointUpdateBaseTest(BaseTest):
	...
	def concurrent_update_eviction_base(self,
	                                    compressed,
	                                    is_chkp_before_insert,
	                                    is_chkp_after_insert,
	                                    loops_count,
	                                    is_loop_update=True):
		node = self.node
		node.append_conf(
		...
		con1.execute(
		    "INSERT INTO o_evicted (SELECT id, id + 1 FROM generate_series(1, 15000, 1) id);\n"
		)
		con1.commit()
